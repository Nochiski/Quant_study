...
from .remote import RemoteEntry, RemoteFS
...
def _reusable_partition(partition: Partition, local_state: TableState | None,
                        ...
                        table: str) -> PartitionReuse | None:
    """로컬 직전 빌드에서 같은 꼬리·같은 content_hash 의 파티션을 찾는다. 파일이 실제로 있어야
    한다."""
...
def _plan_table(remote: RemoteFS, remote_root: str, layer_root: Path, table: str,
                view: ManifestView, state: SyncState, *, reuse: bool) -> TablePlan:
    ...
    for partition in build.partitions:
        ...
        reuse_hit = _reusable_partition(partition, local_state, local_manifest, layer_root, table) \
            if reuse else None
...
def make_plan(remote: RemoteFS, remote_root: str, layer_root: Path, state: SyncState, *,
              layer: str, tables: Iterable[str] | None = None, reuse: bool = True) -> SyncPlan:
    ...
    for table in remote_tables:
        ...
        if view is None:
            continue
        plans.append(_plan_table(remote, remote_root, layer_root, table, view, state, reuse=reuse))
    ...
    return SyncPlan(layer, remote_root, tuple(plans), tuple(meta), tuple(warnings))
