...
| [ ] | `B-03` | `assist-strategy` 사이드바 feature(렌더 안전·취소 확인) | B-02 | `IN_REVIEW` | 구현 완료(로컬 `ae7fb773`, B-02 `bffcacf1` 위 5커밋, 훅 API 적응: exhausted 재연결·rejected 정착·droppedFrames 경고, vitest 714·e2e 19/19) · `review_ai_b_03` 로컬 ref 검토 중 · B-02 최종 `cb852032` 위 재rebase 예정 · 후속 backlog: `ChatMessageView.turn_id` |
