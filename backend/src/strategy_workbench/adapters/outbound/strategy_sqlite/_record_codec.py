...
def encode_record(record: StrategyRevisionRecord) -> tuple[object, ...]:
...
def decode_record(row: sqlite3.Row) -> StrategyRevisionRecord:
    ...
    try:
        ...
        if source_format_text is None and source_text is None and source_hash is None:
            ...
            source_format_text is not None and source_text is not None and source_hash is not None
        ):
