...
- 비고: e2e는 대본 공급자 위에서 돈다. 개념 질문을 보내면 답이 대화에 나오고 문서 원문과 화면이
  그대로라는 경로를 지키며, 실제 모델 설명의 품질은 검사하지 않는다.
...
- 비고: P2-01 의 실행 설정 스키마는 필드마다 설명 키를 낸다(`strategy.field.run_environment.*` 7개,
  키는 backend). frontend 문장과 커버리지 테스트는 BACKLOG-013 으로 P3-02 acceptance 에 예약했다.
  P3-02 가 설명 문장을 붙이면 `예정` 전환을 검토한다(OOS 시작일·연환산 거래일·벤치마크는 실행 설정
  스키마 필드가 아니라 별도 결정이 남는다).
