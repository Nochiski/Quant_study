...
| 편집 이력(undo/redo) | `shared/ui/code-editor`의 CodeMirror history (`CodeEditorHandle`의 `undo`/`redo`/`historyDepth`) | frontend에 병렬 undo 스택을 두지 않는다. `features/edit-strategy/model/use-document-history.ts`는 명령을 중계하고 깊이를 읽기만 하며 자기 이력을 갖지 않고, 탭 밖 버튼(`DocumentHistoryActions`)과 IDE 전역 단축키(Ctrl/⌘+Z·Shift)가 같은 명령을 부른다. 깊이는 표시용이라 실행 게이트로 쓰지 않는다(깊이가 한 프레임 늦은 순간의 입력이 버려지지 않게 — Phase 5 backlog 20과 같은 경합). Form·Graph 트랜잭션 하나 = `replaceRange` 한 번 = 되돌리기 한 단계다. 교체 경로는 둘뿐이고 `documentEpoch`가 가른다. **다른 문서**를 여는 것(epoch 증가, 다른 리비전)만 `loadText`로 교체 자체를 이력에 남기지 않고 앞 문서의 단계까지 버린다 — 되돌리기로 이전 리비전 텍스트에 닿을 수 없다. **같은 문서**의 전체 범위 교체(업그레이드 응답 적용·초안 복구·서버 초안 적용·AI 제안 적용)는 전부 `replaceRange(0, length)` 하나로 통일하며, 되돌릴 수 있는 격리된 한 단계로 남는다(`isolateHistory`) — 격리하지 않으면 직후에 친 글자와 묶여 되돌리기 한 번에 교체분이 통째로 사라진다(P1-02 리뷰 P1). 격리 없는 전체 교체 API는 두지 않는다 |
...
| runtime schema 노드의 필드 표시 사실(type·enum·기본값·범위·format·단위·카탈로그·참조·적용 조건·설명 키) | `features/edit-strategy/model/schema-navigator.ts`의 `schemaFacts`·`referenceCandidates` | Contract Inspector·Form projection이 같은 함수를 호출하고 typed contract 행이 있으면 그 행이 우선한다. authoring·구조 마커(`x-authoring-*`는 스니펫·목록 항목 이름, `x-defines`는 참조 정의 배열 탐색)는 스니펫·projection·outline이 직접 읽는 별개 계약이다. 스니펫 materialize(`materializeSchemaValue`)는 값 생성을 위해 `const`·`enum`·범위·`default`를 직접 읽으며 P4-03에서 `schema-navigator.ts`로 옮겼다. 편집기 완성(`schema-assist.ts`)도 `x-catalog`/`x-reference`를 `schemaFacts`로 읽는다(P4-03, Phase 3 감사 R5). 화면 어휘는 키와 문장을 나눠 소유한다(P1-03, spec D8): 모든 객체·property의 `x-description-key`(i18n 키 **stem** — `<stem>`은 이름, `<stem>.description`은 한 줄 설명)와 연산자 카탈로그 키는 backend가, 로케일별 문장은 소비자가 소유한다. `$defs`는 파이썬 클래스명을 `title`로 내보내지 않으며, frontend는 노드 kind·연산자·property 목록을 손으로 적지 않고 스키마·카탈로그를 순회해 누락 키가 없음을 테스트로 강제한다 |
...
| e2e 실행 잠금·포트·빌드 주소(P1-05) | 포트 번호와 override 환경변수는 `frontend/e2e/ports.mjs`, 머신 단위 잠금은 `frontend/e2e/lock.mjs`(잠금 디렉터리 + `pid` 파일, 자리가 비었을 때만 rename으로 원자 획득, 보유 중에는 `QUANT_E2E_LOCK_HELD`), 빌드 명령과 빌드 자식에게만 주는 `VITE_API_BASE_URL`은 `frontend/e2e/build-command.mjs` | 포트가 비었는지의 판정은 `free-port.mjs` 하나가 하고, `port-owner.mjs`는 누가 물고 있는지 진단만 한다 — 판정에 쓰지 않고 자동으로 죽이지도 않는다. 러너 `run-playwright.mjs`는 `build-command.mjs`의 값으로 잠금 **밖**에서 빌드하고 그 env를 빌드 자식에만 넘긴다. `vite.config.ts`는 `PW_*`를 읽지 않는다 — 읽으면 같은 설정을 쓰는 vitest·dev까지 주소가 바뀌어 MSW 핸들러와 어긋난다(P1-05 1차 리뷰 DEFECT-P105-003). 잠금 순서(유예 검사를 publish 앞에)는 POSIX `rename(2)`가 빈 디렉터리를 덮어쓰는 성질 때문이며 바꾸지 않는다(P1-05 2차 리뷰 DEFECT-P105R2-001). backend CORS 허용 origin은 `STRATEGY_WORKBENCH_ALLOWED_ORIGINS`(`bootstrap/_http.py`)가 읽으며, 포트를 옮기면 이 값도 같이 옮긴다. AI 스택 B-05가 P1-05판을 먼저 main에 들여왔고 lang2 병합(2026-09-27)으로 두 스택이 같은 파일 한 벌을 쓴다. 이 행과 위 파일들만 고친다 |
