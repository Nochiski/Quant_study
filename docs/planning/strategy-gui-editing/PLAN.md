...
last_updated: 2026-09-19T09:59:33+09:00
last_updated: 2026-09-19T09:59:33+09:00
planned_prs: 19
merged_prs: 19
approved_prs: 19
...
| Approved | `19 / 19` |
| Aggregated at | `2026-09-19 09:59 KST` |
<!-- PLAN:SUMMARY:END -->
...
| 8 | ~~P5X-007 — 매뉴얼 스크린샷 전량 재촬영(8절 신규 + 기존 12장 — 마지막 촬영 `5a7b0b3` 2026-09-06이 1.1 전환 `9fab26b` 이전이라 전부 1.0 화면; #143 리뷰 P2-2)~~ → [#148](https://github.com/Nochiski/Quant_study/pull/148) | 두 편집 표면을 글로만 설명, 1·2절 그림이 1.0 구조 | 문서 |
...
| 20 | ~~route 테스트 잔여 flake — 격리(`routes` 프로젝트) 뒤에도 #150 리뷰 14회 중 cold 1회(백테스트 게이트)·warm 1회(Ctrl+S 저장 게이트) 실패. #150이 세 게이트 케이스 모두 compile 도착을 먼저 기다리게 했으나 재현 조건은 미고정. 전체 시간은 직렬화로 98초 → 104~177초~~ → [#155](https://github.com/Nochiski/Quant_study/pull/155) | 실패 시 단언·DOM 스냅샷으로 원인 격리(케이스 timeout 15초 > 예산 10초라 이제 보인다) | 소 |

## 변경 기록

| 시각 | 작성자 | 변경 | 근거 |
|---|---|---|---|
| 2026-09-18 KST | Claude | backlog 20 처리 PR [#155](https://github.com/Nochiski/Quant_study/pull/155) `fix/gui-backlog-20-route-flake`: 원인은 부하가 아니라 `strategy-ide.tsx` 단축키 리스너의 passive effect 교체 틈(버튼 `disabled` commit ↔ 새 닫힌 값 설치 사이의 keydown이 조용히 버려짐) — `useLayoutEffect`로 전환. act 환경을 끈 MutationObserver 회귀 테스트가 `useEffect`에서 red. `--project routes` 6회(cold 1) 전부 초록, Vitest 전체·eslint·tsc·Playwright 19/19. 리뷰 `review_gui_backlog_155` | 머지 뒤 backlog |
