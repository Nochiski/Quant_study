...
`anthropic.AuthenticationError`의 `str()`에는 응답 본문이 들어 있고, 그 본문에는
`Incorrect API key provided: sk-ant-api03-AbC…`처럼 키 조각이 그대로 담긴다. `Failure.message`는
...
def _code_for(error: Exception) -> FailureCode | None:
    ...
    if isinstance(error, anthropic.APIStatusError | anthropic.AnthropicError):
...
def failure_for(error: Exception, *, model: str) -> Failure | None:
    ...
    return Failure(
        code=code,
        message=(
            f"{_MESSAGES[code]} — kind=anthropic model={model} error_type={type(error).__name__}"
        ),
    )
