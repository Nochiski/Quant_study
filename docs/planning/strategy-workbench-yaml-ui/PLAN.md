...
| Head SHA | `09db117` (third code-fix freeze) |
...
|---|---|---|---|---|---|---|
| P6-08 | backend strategy/history/repository 72 + architecture 7; frontend router/legacy save 17 + document routes 46; hostile boundary/cache probes | backend 1,114·frontend 423·Rust 13·root 5; Ruff·Pyright·typecheck·lint·Prettier·build·cargo fmt/clippy | OpenAPI runtime/tracked parity 및 17 files 재생성 전후 SHA-256 동일 | YAML/legacy create/revise가 공용 cancel→remove 사용, 두 late response cache 미복원; duplicate title 두 ID의 toggle/region/pager 이름 고유 | [#74](https://github.com/Nochiski/Quant_study/pull/74) 2차 P1 1/P2 1 수정 완료; 최초 병렬 full의 기존 snippet 5초 flake는 file 46 및 solo full 423 통과; CI billing 차단 | 2026-09-06 |
