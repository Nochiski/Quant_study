...
#     18 GB 를 뜨는 동안 수집 체인이 통째로 막힌다. 03:30 은 어느 체인과도 겹치지 않는다.
#   · **DB 별 격리**: 하나가 실패해도 나머지는 끝까지 간다. 실패 목록을 모아 한 번에 crit.
#   · 사본마다 `PRAGMA integrity_check` 가 `ok` 여야 하고, WAL 잔재(`-wal`·`-shm`)는 지운다.
#   · 보관(사용자 결정 09-14): **주 1회(토요일 03:30, 금요일 장마감분), 최신 1세트만**. 이번 백업이 성공하면 그 밖의
#     세트를 전부 지운다 — 다음 주 백업이 이전 주를 덮어쓰는 셈. 실패한 날은 이전 세트를 남긴다. 최대 2세트(≈38 GB).
#   · 디스크 여유가 60 GB 미만이면 뜨기 전에 crit 후 중단(사본 18 GB × 여유 3배).
...
if [ -z "$FAILED" ]; then
  ...
  for name in $DBS; do
    ...
    echo "──── $name 시작 $(kst) ────"
    timeout "$BACKUP_TIMEOUT" sqlite3 "$SRC" ".backup '$DST'"; BRC=$?
    if [ "$BRC" -ne 0 ]; then
      [ "$BRC" -eq 124 ] && echo "  ! $name .backup 이 $BACKUP_TIMEOUT 안에 끝나지 않았다(외부 쓰기로 재시작 반복 의심)"
      echo "  ! $name .backup 실패(rc=$BRC) — 부분 사본을 지운다"
...
# 보관 정리 — 이번 백업이 성공했을 때만, 최신 KEEP_SETS 세트를 남기고 나머지 YYYYMMDD 세트를 지운다.
if [ -n "$FAILED" ]; then
  echo "──── 보관 정리 건너뜀 — 이번 백업 실패($FAILED), 이전 세트를 남긴다 ────"
else
  echo "──── 보관 정리: 최신 ${KEEP_SETS}세트만 유지 ────"
  n=0
  for dir in $(ls -d "$BACKUP_ROOT"/20[0-9][0-9][01][0-9][0-3][0-9]/ 2>/dev/null | sort -r); do
    base=$(basename "$dir"); n=$((n + 1))
    if [ "$n" -le "$KEEP_SETS" ]; then KEPT=$((KEPT + 1)); continue; fi
    echo "  이전 세트 삭제 $base ($(du -sh "$dir" 2>/dev/null | cut -f1))"
    [ -z "$DRY" ] && rm -rf "$dir"
    PURGED=$((PURGED + 1))
  done
fi
