...
- **Graph**: `그래프 편집` 영역에서 `노드 종류`를 고르고 `노드 추가`를 누르면 새 노드가 생기고 바로
  선택된다. `선택한 노드`에서 `field_id`·`window` 같은 속성과 `input_node_id` 같은 입력 연결을 고치고,
  `그래프 설정`에서 출력 노드(`output_node_id`)와 결측 정책을 정한다. 다른 노드가 입력으로 쓰거나 출력
  노드인 것은 삭제되지 않는다. backend가 아직 실행 계획을 만들지 못한 그래프(노드가 없는 새 팩터 등)도
  편집할 수 있다.
- 두 탭 사이는 Form의 `Graph에서 열기`, Graph의 `Form에서 열기`로 오간다.
- YAML에 구문 오류가 있으면 Form과 Graph는 마지막으로 읽을 수 있었던 값을 `STALE`로 보여 주고 잠긴다.
  `YAML` 탭에서 오류를 고치면 바로 풀린다. JSON 문서는 Form·Graph에서 고치지 않는다.
