...
| e2e 실행 잠금·포트(P1-05가 도입, 아직 없음) | 포트 번호와 override 환경변수는 `frontend/e2e/ports.mjs`, 머신 단위 잠금은 `frontend/e2e/lock.mjs`(잠금 디렉터리 + `pid` 파일, rename으로 원자 획득, 보유 중에는 `QUANT_E2E_LOCK_HELD`) | 포트가 비었는지의 판정은 `free-port.mjs` 하나가 하고, `port-owner.mjs`는 누가 물고 있는지 진단만 한다 — 판정에 쓰지 않고 자동으로 죽이지도 않는다. backend CORS 허용 origin은 `STRATEGY_WORKBENCH_ALLOWED_ORIGINS`(`bootstrap/_http.py`)가 읽으며, 포트를 옮기면 이 값도 같이 옮긴다. AI 스택 B-05는 이 hunk를 그대로 복사해 쓰므로 같은 사실을 두 곳에서 고치지 않는다 |
