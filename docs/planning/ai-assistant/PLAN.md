...
| [ ] | `B-02` | 세션·턴 query, 생성 SDK SSE 리더(재개·멱등), 이벤트 리듀서, property test | B-01 | `IN_REVIEW` | 구현 완료(로컬 `2befae3d`, B-01 최종 `7fcc584e` 위 10커밋) · `review_ai_b_02` 1차 REQUEST_CHANGES 반영(턴별 watermark, `{status 5값, retry()}`, `onSseError` 계수, `AssistantStreamClose` 확장, 규칙 문서) → 2차 재검토 중 · 어휘 단일 입구는 entity · cascade 뒤 push·PR |
