...
> **상태: 완료(09-05, PR #54).** 계획 대비 달라진 것 — `doc_prepass --scan/--repair`(전량 재실행 대체), 연도×분기 샤드, 정제 규칙 5 세 꼴, 검증기 수정 2종, `PARSER_VERSION`. 실행 기록은 `DOC_DESIGN.md` §7 "P1 실측".

> **후속(09-19, 파이프라인 감사 갈래 5).** P1 판은 스냅샷 `snap_20260902T230100Z` 에 동결돼 있었다 — 캐시 키가 스냅샷 id 라 새 스냅샷마다 게이트가 실패해 4표가 매일 skip 됐고, `fin_std` 가 신규 그룹의 `period_end` 정본을 잃었다(DEFECT-F01). 추가된 것: `doc_prepass.incremental(--base-snapshot)` · 캐시 무결성 가드(`build._load_file_source`, DEFECT-F03) · `scripts/doc_prepass_daily.sh`. 설계·비용·gc 보존 규약은 `DOC_DESIGN.md` §7 "일일 증분".
...
- 실행 위치는 `database`. 테스트: `cd database && ../.venv/bin/python -m pytest tests/<file> -q`. 린트: `../.venv/bin/ruff check src/stage/<file> tests/<file>` 와 `../.venv/bin/pyright src/stage/<file> tests/<file>`(둘 다 0 이어야 커밋).
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_vocab.py -q`
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_vocab.py -q && ../.venv/bin/ruff check src/stage/doc_vocab tests/test_stage_doc_vocab.py && ../.venv/bin/pyright src/stage/doc_vocab tests/test_stage_doc_vocab.py`
Expected: `2 passed`, ruff `All checks passed!`, pyright `0 errors`

- [ ] **Step 6: 커밋**

```bash
git add database/src/stage/doc_vocab database/tests/test_stage_doc_vocab.py
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_parsers.py -q -k decode`
...
```bash
git add database/src/stage/parsers_doc.py database/tests/test_stage_doc_parsers.py
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_parsers.py -q -k sanitize`
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_parsers.py -q && ../.venv/bin/ruff check src/stage/parsers_doc.py tests/test_stage_doc_parsers.py && ../.venv/bin/pyright src/stage/parsers_doc.py`
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_parsers.py -q -k parse_tree`
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_parsers.py -q && ../.venv/bin/ruff check src/stage/parsers_doc.py tests/test_stage_doc_parsers.py && ../.venv/bin/pyright src/stage/parsers_doc.py tests/test_stage_doc_parsers.py`
...
Run: `cd database && ../.venv/bin/python -m pytest tests/test_stage_doc_parsers.py -q -k "member_role or generation"`
...
Run: `cd database && ../.venv/bin/python -m pytest tests -q && ../.venv/bin/ruff check src/stage tests && ../.venv/bin/pyright src/stage/model.py src/stage/build.py src/stage/gates.py src/stage/rules_doc.py src/stage/rules.py tests/test_stage_doc_build.py`
Expected: 기존 194 + 새 테스트 전부 통과, 린트 0. 기존 `rules` 선언 대조 테스트(`test_stage_dart.py` 등)가 테이블 수를 세면 4 늘어난 값으로 갱신한다.

- [ ] **Step 5: 커밋**

```bash
git add database/src/stage database/tests/test_stage_doc_build.py
...
- [ ] **Step 1: 배포** — 로컬 전체 테스트·린트 통과 확인 뒤 `rsync -avz --exclude='.venv' --exclude='__pycache__' database/src/ kael-server:~/quant-ledger/src/` 와 `scripts/`.
...
## 계획 검토 반영 (2026-09-04, 검토자가 계획 코드를 조립·실행해 확인한 19건)

구현 시 아래를 계획 본문보다 우선한다.
1. **픽스처 네임스페이스**: 테스트 XML 의 `xmlns:xsi="x"` 를 실제 URI `http://www.w3.org/2001/XMLSchema-instance` 로. `generation()` 은 `root.attrib` 키를 `k.upper().endswith("NONAMESPACESCHEMALOCATION")` 로 찾는다(lenient 트리의 `XSI:NONAMESPACESCHEMALOCATION` 도 잡힘).
2. **깨진 ZIP 은 D0**: `parse_zip` 의 BadZipFile 행은 D2 분모·분자에서 빼고 `n_zip_open_failed` 로 D0(폐기형)에 계상. Task 10 "ok" 픽스처에서 깨진 ZIP 제거(→ parse_log 3, modes {ok 2, html 1}); 깨진 ZIP 은 별도 테스트로 `status == "gate_failed"`·`"D0" in detail`.
3. **기대값**: Task 9·11 `n_xbrl_groups` = 3 (BODY_TOC 의 `{XBRL}BS` + BODY_TABLES 2). Task 12 `n_corr_rows` = 1, d12 `n_docs` = 2(ok 행만).
4. **lenient 와 `<TITLE>`**: `_LenientBuilder` 에 `CDATA_CONTENT_ELEMENTS = ()`·`RCDATA_CONTENT_ELEMENTS = ()` 클래스 속성(파이썬 3.12.13 html.parser 는 TITLE 을 RCDATA 로 읽는다). TITLE 안 SPAN 이 있는 lenient 테스트 추가.
5. **엔티티 보존 표기**: `ent()` 가 `f"[&amp;{name};]"` 를 직접 낸다(규칙 3 의 `n_bare_amp` 오염 방지). 기대값 `[&amp;nbsp;]`.
6. **정정 표 헤더·날짜**: 정정사항 표 헤더 키는 공백 전부 제거(`항목`·`정정사유`·`정정전`·`정정후`·`정정요구ㆍ명령관련여부`). `filed_raw` 는 앵커 뒤 날짜 정규식 매치 끝까지에 `\s*일` 옵션 포함. `filed_date` 는 `datetime.date(y, m, d)` 로 달력 검증, 실패 시 `None`·`unparsed`(§1.7 오기 실측: 13월·45일이 G2 폐기를 부르지 않도록).
7. **§3 컬럼 보강**: `stg_doc_section.elem_end`(`idx[scope] + 요소 수 − 1`), `stg_doc_parse_log.t_decode_ms·t_sanitize_ms·t_parse_ms`(`perf_counter`). `page_found` 는 상수 열이라 §3.3 에서 "ZIP 은 있으나 CORRECTION 없음" 은 meta 의 `has_correction_page=false` 로 보고 이 열은 유지만; `unknown_aclass`·`unknown_acode_n` 은 P3 사전 뒤로 — Task 13 문서 갱신에 컬럼별로 명시.
8. **ZIP 디렉토리 엔트리**: `zf.infolist()` 에서 `is_dir()` 건너뜀. 같은 ZIP 의 두 멤버가 모두 CORRECTION 을 가지면 `stg_doc_correction` 은 main 우선 1행(§3.3 키 `(rcept_no,)` 유지) — `parse_zip` 에서 접는다.
9. **프리패스 재실행**: `run()` 시작 시 `<cache>/<table>/` 의 `year=*.jsonl` 을 지운다(부분 연도 재실행이 stale 파일을 남겨 G8 등식을 깨뜨림).
10. **기존 테스트 갱신**: `tests/test_stage_model.py::test_registry_assembles_per_source_modules` 의 `mods` 에 `rules_doc` 추가(Task 11 파일 목록).
11. **린트**: 줄 폭 100 초과 37곳 정리, 테스트 파일의 중간 import 를 상단으로(E402·I001), pyright Optional 접근은 지역 변수 + `assert … is not None`, `doc_checks.run` 반환은 `@dataclass` `DocCheckReport`(`d6`·`d8`·`d12`·`status`·`detail`) 로.
12. **D2·D3 분모**: 문서(ZIP) 단위로 통일 — `modes` 는 main 멤버(없으면 첫 멤버) 의 parse_mode, D3 는 문서 빈도. `summary` 에 `n_docs = ok+lenient+failed+html` 등식을 단언.
13. **BOM**: `decode_bytes` 가 `text.lstrip("\ufeff")` — 텍스트 등식이 BOM 으로 깨지지 않게.
14. **죽은 계산**: `table_census` 의 `n_form`(EXTRACTION 표 수)을 `n_form_tables` 로 meta 에 싣는다(rules_doc 선언에도 추가).
15. **P1 에서 미루는 설계 항목**: `survey/doc_census.py`(P3 사전 census), 카탈로그 뷰(결정 ⑬, P1 마지막 태스크로 추가: `data/stage/catalog.duckdb` 에 MANIFEST current_build 뷰 생성 스크립트 `scripts/stage_catalog.py`), D5 스냅샷 해시(`_meta.json` 에 `doc_input_hash` 기록 — `_load_file_source` 가 summary 의 해시를 넘김), D1 두 번째 절(rcept_no 당 meta ≥1행 — `doc_checks` 에 추가).
16. `doc_checks` docstring 과 코드 일치(기록형만, `status` 는 D1 위반 시에만 `gate_failed`). `STG_DOC_SECTION` 재생성 3줄 삭제, `_n(name, key=False)` 로.

## 후속 (별도 계획)
- **일일 증분** (09-19 완료, 갈래 5): `--base-snapshot` 증분 · F03 가드 · `doc_prepass_daily.sh`. 서버 잔여 — 09-19 전량 프리패스 1회 → 4표 수동 빌드 → 체인 훅(`build_chain.sh` 의 stage 전량 앞) · `gc.sh` 캐시 보존(R9 개정) · D10 을 문서 단위 격리형으로 낮출지 결정 · `doc_checks` 호출 지점.
