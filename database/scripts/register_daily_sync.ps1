...
#
#   database\scripts\register_daily_sync.ps1                 # 등록 (기본 10:00 KST — 아침 확정판 m_ 09:19 KST 뒤)
...
# `ledger_sync.ps1 status --remote`. 작업은 로그온한 사용자 컨텍스트로 돌며(`Register-ScheduledTask`
# 기본), 로그아웃 상태에서도 돌리려면 `-User`/`-Password` 또는 gMSA 로 다시 등록한다.
param(
    [string]$At = "10:00",
