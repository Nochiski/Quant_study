...
| 결합 전 정규화 방식과 합성 공식 | `domain/portfolio/_compiler.py` 가 `spec.signal.normalization` 을 읽어 `composite = Σ(sign(direction) × weight × norm(x)) / Σ|weight|` 를 계산한다(P2-04, spec D4). `norm` 의 횡단면 순위·표준화 공식 자체는 `domain/factor/_statistics.py` 의 `cross_sectional_rank`·`cross_sectional_zscore` 하나씩이 소유하고, 팩터 그래프의 `CrossSectionalOperator` 도 같은 함수를 부른다 — 같은 문서가 두 가지 순위를 내지 않는다 | 정규화 모집단은 팩터 그래프 횡단면 연산자와 같은 동료 집단 규칙을 쓴다: 한 리밸런싱 프레임은 기준일 하나이므로 `universe_member` 가 같은 행끼리만 서로의 순위에 들어간다. 결측(`missing` 정책 적용 뒤에도 `None`)·공개일이 기준일보다 늦은 값·비유한값은 모집단에서 빠지며, 이는 `_score_candidate` 가 같은 값을 점수에서 버리는 사유(`MISSING_FACTOR`·`FUTURE_DATA`·예외)와 같다. 즉 순서는 항상 결측 처리 → 정규화 → 가중 합이다. `normalization` 은 canonical payload에 들어가므로 `spec_hash`·`tape_hash` 가 값마다 다르다. frontend 는 값을 문장으로 옮기는 i18n 만 갖고(P3-01) 공식을 복제하지 않는다. `weighting: factor_score` 의 비중 규칙은 `_compiler.py` 의 `_margin_strengths` 하나가 소유한다: 선정이 2종목 이상이면 선정 종목 강도 = 합성 점수 − 기준점, 기준점 = `min(선정 최저 점수 이하인 eligible 비선정 종목 중 최고 점수, 선정 최저 − (선정 최고 − 선정 최저) / (선정 수 − 1))`(컷 아래 종목이 없으면 뒤 항), 선정 1종목이거나 강도가 모두 0 이면 균등 배분, 숏은 합성 점수 부호를 뒤집어 같은 규칙(PLAN P2-04 결정 5). 합성 점수 절댓값, eligible 최저 점수, 컷 아래 최고만을 기준점으로 쓰지 않는다 — 차례로 `direction: low`·공매도 쪽 순서 반전, 선정 종목 비중 0, 근접 동점 dust 를 만든다 |
...
| 실행 설정(시장·빈도·기간·유니버스·체결·수수료·결측) | `domain/backtest/_models.py`의 `RunEnvironment` (P2-01 모델, P2-02 `결측`, P2-03에서 전략 문서의 `data`·`execution`·`graph.missing_policy` 제거 완료). `Market`·`DataFrequency`·`ExecutionTiming` enum과 수치 범위 행(`RUN_ENVIRONMENT_CONSTRAINTS`)·유니버스 카탈로그 마커(`CATALOG_UNIVERSE`)도 이 노드가 소유한다(P2-03) | 전략 revision에 저장하지 않고 run manifest에 `environment`·`environment_hash`로 기록한다. frontend 실행 설정 패널이 편집 값을 소유하며 기본값은 실행 설정 스키마 `GET /api/v1/run-environments/schema`에서만 읽는다(전략 authoring runtime schema와 다른 산출물, 프론트에 적지 않는다). 1.2 문서에는 이 값을 담을 자리가 없으므로 실행 요청(preview·trace·run)이 반드시 실어야 한다 — 없으면 `domain/backtest`의 `require_environment`가 코드화된 진단(`run_environment.required`, run 경로는 422 `backtest.run.environment_required`)으로 거절하고 기본값을 지어내지 않는다. 1.1 문서에서 값을 꺼내던 브리지(`environment_from_legacy_spec`)는 입력이 사라져 P2-03에서 삭제했고, 은퇴 버전 문서의 환경은 업그레이더 응답이 돌려준다(P2-09, spec D7). 결측 정책의 owner도 `RunEnvironment.missing` 하나다(P2-02). 실행 설정을 갖지 않는 팩터 sandbox(`/factors/explain`·`/factors/preview`) 요청이 `missing`을 생략하면 같은 노드의 `DEFAULT_MISSING_POLICY`를 쓴다 — 두 기본값이 갈리면 편집 화면의 실행 플랜 패널이 실제 실행과 다른 `plan_hash`를 보인다(P2-03): `compile_factor_plan`과 `evaluate_factor_graph`가 `missing`을 **인자로** 받고 그 값이 `plan_hash`에 남는다. 참여율(`engine_portfolio`의 능력 판정)과 체결 시점(`compile_target_tape`의 tape hash)도 실행 설정에서 인자로 받는다(P2-03). `spec.data.*`·`spec.execution.*`·`graph.missing_policy` 읽기가 `src` 전체에서 0건인지는 `tests/architecture/test_run_environment_ownership.py`가 고정한다 |
...
| authoring schema 버전 | `domain/strategy/_models.py`의 `CURRENT_SCHEMA_VERSION`(현재 `"1.2"`, `_hydrate.py`의 `SUPPORTED_SCHEMA_VERSIONS`가 파생), 은퇴 버전은 `domain/strategy/_upgrade.py`가 소유한다(현재 `LEGACY_SCHEMA_VERSION` 단수 상수 + 1.0 step의 목표 버전 `LEGACY_UPGRADE_TARGET_VERSION`. `FROZEN_SCHEMA_VERSIONS` 집합 + 버전 디스패치 API로 바뀐다 — P2-09, 아직 없음) | 모델 기본값·스키마·검증·어댑터·테스트는 이 상수를 읽는다. `"1.1"`/`"1.0"` 리터럴을 다시 적지 않는다. frontend도 같다: 동결/업그레이드 판정은 `requires_upgrade`와 compile 진단으로만 하고 은퇴 버전 문자열을 갖지 않는다. 버전 리터럴을 남겨야 하면(새 문서 템플릿) 그 값이 runtime schema `schema_version.const`와 같은지 단언하는 테스트를 같은 PR에 넣는다 |
| 1.0 → 1.1 → 1.2 문서 업그레이드 변환(1.1 → 1.2 step 등록과 버전 디스패치는 P2-09에서 추가, 아직 없음) | `domain/strategy/_upgrade.py`의 `UPGRADE_STEPS`(현재 1.0 → 1.1 평탄 튜플)와 1.1 → 1.2 내용 변환 `strip_retired_execution_settings`. P2-09가 `from_version → steps` 맵으로 바꾸고 공개 API `upgrade_document(tree) -> UpgradeOutcome` 하나가 중간 버전을 순서대로 적용·검증한다 | dict 경로(repository codec, legacy generated source)와 source 경로(`adapters/outbound/document_codec/_upgrade_source.py`)가 같은 step을 적용한다. P2-09 전까지 source 경로는 1.1 에서 멈추므로 `POST /strategy-documents/upgrade` 결과는 아직 저장·실행할 수 없고, 저장된 은퇴 버전 row 만 repository codec 이 `strip_retired_execution_settings` 까지 태워 현재 버전으로 읽는다. 어댑터는 주석·순서 보존만 맡고, 두 경로가 다른 tree를 내면 application이 `strategy_document.upgrade_drift` 422로 거부한다. frontend는 변환 규칙을 알지 않고 응답 원문을 그대로 적용한다 |
...
| 편집기 source 트랜잭션(범위 계산·들여쓰기 폭·EOL·빈 컨테이너 표기) | `features/edit-strategy/model/source-transactions.ts`의 `planSourceOperation` | 범위는 `shared/lib/yaml12`의 `parseSource`가 낸 `valueRanges`/`keyRanges` offset을 그대로 쓰고 CST를 다시 걷지 않는다. 모든 연산은 완성된 다음 원문을 `parseSource`로 preflight하고 tree가 `applyToTree` 결과와 같을 때만 성공을 반환한다. 스니펫·Form·Graph는 이 함수에 연산을 넘기기만 하고 fragment 문자열을 직접 조립하지 않는다. 연산 여러 개(빈 그래프의 첫 노드 + 출력 지정)는 `planSourceOperations`가 같은 함수로 순차 계획해 편집 한 번으로 합친다. flow 표기(`{ … }`·`[ … ]`) 컨테이너에 키·항목을 넣거나 거기서 지울 때는 가장 바깥 flow 컨테이너부터 block으로 다시 쓰고, 지워서 비면 `{}`/`[]`를 그 자리에 둔다(루트 flow 포함). flow 안 주석은 보존하지 않고 컨테이너 뒤 줄 끝 주석·`key:`/`-` 줄·그 사이 자기 줄 주석은 남기며, preflight가 tree 동치를 검증한다. source 트랜잭션의 편집기 적용(`replaceRange` 한 번·feedback scope)은 `use-source-transactions.ts`의 `useSourceTransactions` 하나가 하고, 스니펫 훅은 그 `run`을 쓴다(backend 업그레이드 응답 적용 `use-upgrade-document.ts`와 AI 어시스턴트 제안 적용(2026-09-20 AI 어시스턴트 설계 D7)은 아래 '금지' 절이 따로 허용한 전체 범위 교체다 — 둘 다 `replaceRange(0, length, text)`로 history를 격리한다. 업그레이드는 응답 도착 시 편집기 텍스트가 요청 시점과 다르면 적용하지 않고, 제안은 다르면 사용자 확인(미리보기·덮어쓰기)을 거친다). Form·Graph는 `enabled`가 참일 때만 연산을 넘긴다(계획은 편집기 live 텍스트로 세우므로 stale pointer는 preflight가 잡지 못한다). 편집기(CodeMirror 적재)는 CRLF를 LF로 정규화하고 보존하지 않는다 — 편집 없이 저장해도 `source_hash`는 바뀔 수 있고 `spec_hash`는 불변이다(Phase 5 감사 P5X-005 결정, 매뉴얼 4절 안내). `planSourceOperation`의 `detectEol`(`Eol`)은 CRLF를 감지·보존하지만 계획 입력이 항상 편집기 live 텍스트라 실전에서는 `\n`만 관측된다 — 편집기를 우회하는 적재 경로가 생기면 그때 유효해지는 방어이며(P3-01 리뷰 P2-9), 죽은 코드가 아니다 |
...
| AI 어시스턴트 도구 정의·시스템 프롬프트·제안 검증 | `application/assistant_chat`(`_prompt.py`, 도구 실행), 도구 이름·스키마 상수는 `domain/assistant` | 공급자 adapter(`llm_anthropic`·`llm_openai`)는 `ToolSpec`을 자기 형식으로 변환하고 루프만 돈다. 공급자 SDK import는 두 adapter 밖 금지(architecture 테스트). **모델에게 가는 문장은 adapter가 저술하지 않는다** — `llm_openai`가 검색 상한에 닿아 `web_search`를 뺄 때 모델과 화면에 보내는 문구도 `_prompt.py`의 `SEARCH_BUDGET_EXHAUSTED_NOTICE`를 그대로 실어 나른다. **어떤 adapter가 설치되었는가**는 composition root만 정한다 — `bootstrap/_assistant.py`의 `PROVIDER_ADAPTER_FACTORIES`가 정본이고, 브라우저 e2e용 대본 adapter(`adapters/outbound/llm_scripted`)로의 교체는 `bootstrap/_http.py`가 환경 변수 `STRATEGY_WORKBENCH_ASSISTANT_FAKE_PROVIDER=1`을 읽었을 때만 일어난다(기본 off, 그 값 하나만 켠다). 화면·application·adapter는 가짜인지 진짜인지 묻지 않는다 |
| AI 어시스턴트 턴 상한(라운드·검색·토큰 예산)의 **값** / **집행** | 값은 `domain/assistant/_models.py`의 `DEFAULT_*` 상수와 `application/assistant_chat`이 채우는 `TurnRequest`, 집행은 공급자 adapter | 루프의 주인이 adapter이고 `TurnRequest`는 루프 시작 전에 한 번 넘어가므로 서비스가 도중에 호출당 상한을 낮출 통로가 없다(설계 spec D3). adapter가 라운드·출력 토큰을 세어 다음 호출의 `max_tokens`를 줄이거나 `TOOL_ROUNDS_EXCEEDED`·`TOKEN_BUDGET_EXCEEDED`로 멈춘다. 서비스는 `Usage`를 기록만 하고 라운드를 세지 않는다. 턴 타임아웃만 `AssistantTurnRunner`가 집행한다 |
| AI 채팅 세션·턴·메시지·이벤트 이력(sequence) | `assistant_sqlite` | 진행 중 턴 레지스트리·취소 신호는 `application/assistant_chat`의 `AssistantTurnRunner`(프로세스 내, 단일 워커 전제). 사이드바 열림·폭·현재 세션 id·제안 카드의 기준 텍스트와 **마지막 반영 sequence·턴 투영**(`entities/assistant` 리듀서, 사유는 `.claude/rules/frontend-api-state.md`)은 frontend local UI state. 제안은 사용자의 "적용"으로만, 업그레이드 적용과 같은 편집기 전체 범위 교체(`replaceRange`, 문서가 바뀌었으면 적용 전 확인) 경로로 문서에 들어간다 — source 트랜잭션 행의 두 번째 명시 예외. `ChatEvent`에는 판별자가 없으므로 전송용 `type` 태그와 그 합집합 메타데이터는 `adapters/inbound/http_api/_assistant_contract.py`가 소유한다 — domain은 `isinstance`로, 생성 SDK는 `type`으로 갈래를 가른다 |
