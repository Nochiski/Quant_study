 ...
 *
 * 제목은 backend가 발행한 이름을 보이고 스키마 키는 보조 `<code>`로 내려간다(P1-03). 루트
 * 스칼라 섹션처럼 스키마 키가 없는 자리는 이름만 보인다. 이름도 키도 없는 마지막 분기는
 * 번역이 빠졌을 때의 폴백이다 — `screen-vocabulary.test.ts`가 누락 키를 막는 동안에는 도달하지
 * 않지만, 그때 빈 버튼을 보이는 것보다 키라도 보이는 편이 낫다.
 */
const SectionToggle = ({
  schemaKey,
  descriptionKey,
