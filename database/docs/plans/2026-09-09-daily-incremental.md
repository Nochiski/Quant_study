...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-09-09 18:50 — 키움 갭 즉시 실행(사용자 결정), 크론에 키움 단계 포함. PR #96 |
...
| P2 갭 메우기 | **키움 제외 완료**(09-09 18:30) — 2.1 KRX 13일·2.3 KIS(신규 33,513행)·2.4 DART 08-20~09-08 15일 전부 게이트 통과·2.5 유니버스 3,483. G2 #1(키움 제외)·#2·#4·#5 통과. **2.2 키움은 토요일 09-12 11:00 일회성 크론** → 그 뒤 G2 #3·#6 |
| P3 원장 크론·관찰 | **가동**(09-09 18:10 등록, 18:50 키움 단계 포함으로 변경 — `QL_SKIP_KW` 해제) — 첫 실행 09-10 06:00·08:10 KST. 5거래일 관찰 → G3 |
| P4 stage 일일 전량 | 미착수 |
...
| 3 | 오염 | 토요일 키움 실행 후 판정: `ka10008` 08-24 stale ≤ 30%, 원장에 `dt > T-1` 행 0 |
| 4 | KIS 중복 | **✓** payload 동일 중복쌍 0 (KIS 실행 후 실측) |
| 5 | 예산·키 | **✓** 갭 메우기 DART 총 12,954콜 < 15,000, `kael` 0 |
| 6 | 키움 유니버스 | 토요일 실행 후 판정: 요청 유니버스 = `tickers.txt ∪ kiwoom_common`, 386380 포함, 차집합 ≈40종목 로그 존재 |
