...
> `docs/archive/BACKFILL_REVIEW.md`. 이전 판(206,859콜 · 10.61일)은 ① 모수가 3,341 이었고
...
| **P3h** | 감사의견 `accnutAdtor…` | 26,811 (전 연도) | **26,811** | 1.4 | **신규.** E08. 1콜에 3개 연도가 오지만 3년-stride 는 게이팅(`lo<=y<=hi`)과 결합 시 상장창이 stride 연도를 안 지나는 기업이 0콜이 되는 커버리지 구멍을 만든다 (2026-08-27 검토) — 전 연도 수집으로 확정. 3연도 중복 행은 req_bsns_year 포함 해시로 원장 무손실 |
...
### 3-1-2. 실한도·한도 창 — 실측 확정 (2026-08-26)

공식 안내가 "일반적으로 20,000건 이상"이라 실한도와 리셋 방식이 미확정이었다. 둘 다 실측했다.

| 항목 | 공식 안내 | **실측** | 근거 |
|---|---|---|---|
| 일 한도 | "20,000건 이상" | **40,000콜** | k2 가 40,000번째까지 전건 정상, 40,001번째에 `020` |
| 리셋 방식 | 미기재 | **KST 자정 리셋** | 25,000콜 쓴 k2 가 자정 직후 `status=000` |

**측정이 가능했던 이유**는 사전 계상 상한을 없앴기 때문이다. 임의값(19,800·25,000)으로
막아 두는 동안에는 우리가 먼저 비켜서서 DART 가 거부하는 지점을 볼 수 없었다.

`QUOTA_LIMIT` 은 이제 40,000 이다. 이 값은 **`020` 이 "우리 소진"인지 가르는 기준**이지
사전 정지선이 아니다. 20,000 으로 두면 20,000~40,000 구간의 `020` 을 우리 소진으로
오판해 프로브 없이 키를 접는다 — 그 구간의 `020` 은 오히려 버스트 한도나 외부 소진을
의심해야 하는 신호다.
...
| 키 | 사전 상한 | 근거 |
|---|---|---|
| k2, k3 (우리 키) | **없음** | 020 이 올 때까지 쓴다. 임의값은 실한도와의 차이만큼 매일 버린다 |
| kael (프로덕션) | **39,000** | 실한도 40,000 − 카엘 시스템 몫(일 ~300콜) − 마진 |

**실전 동작 확인** — 2026-08-26 23:32:46, k2 40,000번째 콜에서 `020` 수신 후
**같은 초에** k3 첫 콜이 나갔다. 사용량이 임계를 넘긴 상태라 프로브 없이 `ours` 판정.
대기 0초.
...
DART_HOME=/Users/claudeoscarmonet/Desktop/Quant_study/database
PY=/usr/bin/python3
...
- **위험성**: **데이터 손상은 아니다.** 원장은 append 전용이고 인덱스가 없는 편이 쓰기에 유리하므로 이 상태 자체는 설계 의도에 부합한다(§1.9 가 `dart_call_log` 에 대해 같은 근거를 명시한다). 문제는 **소비 규약이 문서에 없다**는 것이다:
  ...
  · 스테이지(`build_stage.py`)는 `ix_fact_concept`·`ix_fact_corp` 를 만들지만, **스테이지를 거치지 않고 원장에 직접 붙는 코드를 막는 장치가 없다.** KRX 원장에서 같은 실수가 실제로 위험했다(`archive/SOURCE_AUDIT.md` RISK-1·2 — 원장 직접 조회 시 조인 0행·사전순 정렬)
