...
| `universe_id` | `krx.all`(정책 미적용) · **`krx.common-stock`**(`sec_type='common'` ∧ `status='listed'` — S03B `universe_policy` 행, 계약 테스트 `backend/tests/contract/test_raw_observation_port.py:33` 의 값) · `krx.investable`(S03B 플래그 4행: common-stock + `NOT admin_state` + `NOT liquidation_window`) · `krx.liquid`(S03B-2, 09-05 결정 "날짜별 상위 비율": investable 4행 + `adv20_rank_pct >= 1 − liquid_top_pct` quantile 1행 — `adv20_rank_pct` 는 같은 날 보통주·listed·adv20 있음 모집단 안 `adv20_krw` 의 cume_dist, baseline `universe_policy.liquid_top_pct` 0.5 = 상위 50%) | 계약 테스트 |
