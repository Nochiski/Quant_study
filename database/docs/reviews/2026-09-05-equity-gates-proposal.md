# Equity 층 게이트 명세 `docs/EQUITY_GATES.md` (초안 v0.1, 2026-09-05)
...
| 항 | 테스트 ID | 실행 방법 | 상수 |
|---|---|---|---|
| ① | `EGC-01` | 엔진 저장소 `tests/test_bar_source_contract.py::BUILDERS` 에 `equity_duckdb` 등록 → 전 케이스 통과. **엔진은 별도 저장소** → equity CI 는 엔진을 서브모듈/설치 의존성으로 잡고, 없으면 `skip(engine_absent)` | — |
...
| A13 | WORKFLOW §2 EG-C ① | "`tests/test_bar_source_contract.py::BUILDERS` 등록" | 엔진은 **별도 저장소**. equity CI 가 그 파일을 못 볼 수 있다 | 술어화 | 엔진 의존을 명시하고 부재 시 `skip(engine_absent)` — 단 7단계 통과 조건에서는 skip 불가 |
...
| C17 | WORKFLOW §3-5 입력 | `stg_consensus_monthly/annual/quarterly/matrix` | `rules_wise.py` 에 실재하는 것은 `stg_consensus_monthly` 뿐 | 술어화 | 입력 목록 정정. EG0-P02 가 실제로 잡는다 |
...
|---|---|---|
| 유지 | 4 | C3·C9·C10·B3 |
| 술어화 | 27 | A1·A4·A7~A14·B1·B2·B4~B8·C4·C5·C7·C8·C11~C13·C15·C17 |
