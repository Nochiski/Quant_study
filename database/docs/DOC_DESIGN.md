...
### 1.4 ZIP 멤버 — 이름으로 고른다
- 멤버명 = `{rcept_no}.xml`(본문) · `{rcept_no}_00760.xml`(감사보고서, ACODE 00760) · `{rcept_no}_00761.xml`(연결감사보고서, 00761). **2015~2016 은 `/{rcept_no}.xml` 처럼 `/` 접두** — basename 으로 매칭. Y2040 전량에서 `_{rcept_no}.xml`(밑줄 접두) 본문 1건(한화 2024 사업보고서) — 정규식이 `_?` 를 허용한다.
...
- 사용자 결정(09-03): Claude API 미사용, 판정은 이 Claude Code 세션(서브에이전트 활용은 세션 판단). 골든셋 100표 = `database/eval/table_schema/`(`cases.jsonl`·`cases.md`·`labels_table.csv`·`labels_columns.csv`·`README.md`; 층화 = 사업의 내용 40·주석 30·기타 30 × G1 50·G2 20·G3 30, 2단 헤더 66, 단위 미표기 76, 회사 39곳, 시드 20260903). 입력 승인·라벨링 대기, 분기·반기 표 10~15개 교체 권고.
...
| # | 이름 | 분류 | 술어 | 임계(초기·근거) |
|---|---|---|---|---|
| D0 | 실물 계약 | 폐기형 | 프리패스 입력 = 스냅샷 `doc_store.zip_ok=1` 전건 · 멤버 basename 정규식 `^_?\d{14}(_\d{5})?\.xml$` 밖은 `role=other`(Y2040: 0건) · ZIP 열기 실패 0 | 위반 0 |
...
**P1 실측 (스냅샷 `snap_20260902T230100Z`, 파서 p1.4, 캐시 3.7GB·분기 샤드 68)**

| 항목 | 값 |
|---|---|
| 프리패스 | 171,179 문서 · 멤버 242,196 · ZIP 결함 0 · 파싱 실패 0 · expat 정상 169,290 · lenient 65 · HTML 1,824 · 3차 4h34m(4워커, RSS 0.59GB) |
| 게이트 | D0·D2·D3·D10 전부 통과(텍스트 등식 위반 0). 정제 시간이 멤버당 117ms 로 파싱(40ms)의 3배 — 규칙 5c 정규식, 미결 |
| `stg_doc_meta` | 242,196행 · 6.7s · RSS 2.2GB · G0~G9 pass(G4 234/234) · 재빌드 hash 동일 · 22MB |
| `stg_doc_section` | 7,929,624행 · 74s · RSS 6.7GB · pass · 재빌드 hash 동일 · 111MB |
| `stg_doc_correction` | 17,600행 · 1.2s · pass(1차 빌드 G2 fail 1건 `20011년 5월 16일` → 파서 수정 후 `--repair` 22s·RSS 60MB) · 5.5MB |
| `stg_doc_parse_log` | 242,196행 · 2.6s · pass |
| D6 | main·파싱 ok 168,938 문서 중 첫 장 보유 17,176 · correction 행 17,188 · `filed_date` 해석 14,856(86.4%, C340 의 86.6% 와 일치) |
| D8 | main 사업보고서 중 XBRL 그룹 ≥4 비율: 2010 0.94 → 2016 0.81(최저) → 2024~2026 0.94~0.95 (§1.13 의 0.76~0.96 안) |
| D12 | 멤버 240,372 중 미등록 토큰 보유 11,752(4.9%, 전부 `<LED>`·`<IT>` 류 본문) · lenient 65 · `section_kind=NULL` 은 3단계 소제목(2024 샤드 30%) |
| 픽스처 정의 | `toc_n` = §1.1 + 1(표지 포함), `n_xbrl_groups` = 문서 전체(2013-B 8 vs 6) — §4 D4 |
