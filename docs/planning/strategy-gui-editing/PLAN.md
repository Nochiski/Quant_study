...
last_updated: 2026-09-19T00:31:41+09:00
last_updated: 2026-09-19T00:31:41+09:00
planned_prs: 19
merged_prs: 19
approved_prs: 19
...
| Approved | `19 / 19` |
| Aggregated at | `2026-09-19 00:31 KST` |
<!-- PLAN:SUMMARY:END -->
...
| 19 | ~~route 테스트 부하 flake의 근본은 54파일 병렬의 워커 경합(단독 65초·케이스당 1초) — #149는 전역 비동기 예산을 10초로 올려 완화만 했다. 파일 분할 또는 워커 격리(vitest projects)로 근본 해결~~ → [#150](https://github.com/Nochiski/Quant_study/pull/150) | #149 리뷰 P1-1 | 중 |

## 변경 기록

| 시각 | 작성자 | 변경 | 근거 |
|---|---|---|---|
| 2026-09-18 KST | Claude | backlog 18·19 처리 PR [#150](https://github.com/Nochiski/Quant_study/pull/150) `fix/gui-backlog-c2`(main 기반): `planRemove`도 flow 부모면 `replaceFlowContainer`(빈 결과는 `{}`/`[]` 인라인), vitest `projects`(unit / routes, `groupOrder: 1`)로 route 파일을 나머지 뒤에 혼자 실행. Vitest 626/54·eslint·tsc·Playwright 19/19. 리뷰 `review_gui_backlog_150` | 머지 뒤 backlog |
