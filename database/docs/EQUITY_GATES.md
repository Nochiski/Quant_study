...
| 격리 단위 | — | **원장 행만 격리하고 격자 셀은 지우지 않는다.** 그 (date, ticker) 셀은 17축 전부 NULL 로 남아 ⑪(a) 행수 등식이 유지된다(셀째로 버리면 그날 그 종목이 유니버스에서 사라져 생존편향). SQL 은 `base` 에 `drop_value` 플래그를 세우고 `LEFT JOIN … AND NOT b.drop_value` 조인 조건 하나로 값을 비운다 | `sql/credit_daily.sql` `over`·`base`·`cell` |
