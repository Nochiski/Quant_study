...
test.describe("AI 어시스턴트", () => {
  ...
  test(
    "설정에서 공급자를 등록하면 활성이 되고 키는 꼬리 4자리만 남는다",
    { tag: ["@story", "@US-DM-01"] },
    async ({ page }) => {
      ...
      await expect(
        section.getByTestId("provider-kind-anthropic"),
      ).toContainText("사용 가능");
  ...
  test(
    "사이드바 질문에 답이 스트리밍되고 새로고침해도 이력과 진행 중 턴이 이어진다",
    { tag: ["@story", "@US-DM-02"] },
    async ({ page }) => {
  ...
  test(
    "팩터 그래프를 바꾸는 제안도 적용 후 백테스트가 팩터 계획 조회를 기다려 실행한다",
    { tag: ["@story", "@US-CS-04"] },
    async ({ page }) => {
  ...
  test(
    "검색 출처를 링크로 보이고 검증에 실패한 턴은 실패 문구로 끝난다",
    { tag: ["@story", "@US-CS-04"] },
    async ({ page }) => {
      await ensureProvider(page);
