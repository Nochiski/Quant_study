...
- **Graph**: `그래프 편집` 영역의 `노드`에서 종류를 고르고 `노드 추가`를 누르면 새 노드가 생기고 바로
  선택된다. 종류 목록과 노드 카드는 `기간 집계`처럼 사람 말 이름을 보이고 `time_series` 같은 키는
  옆에 남는다. 연산을 고르면 그 연산의 한 줄 설명과 계산식이 필드 아래에 나온다.
