...
| 편집 이력(undo/redo) | `shared/ui/code-editor`의 CodeMirror history (`CodeEditorHandle`의 `undo`/`redo`/`historyDepth`) | frontend에 병렬 undo 스택을 두지 않는다. `features/edit-strategy/model/use-document-history.ts`는 명령을 중계하고 깊이를 읽기만 하며 자기 이력을 갖지 않고, 탭 밖 버튼(`DocumentHistoryActions`)과 IDE 전역 단축키(Ctrl/⌘+Z·Shift)가 같은 명령을 부른다. 깊이는 표시용이라 실행 게이트로 쓰지 않는다(깊이가 한 프레임 늦은 순간의 입력이 버려지지 않게 — Phase 5 backlog 20과 같은 경합). Form·Graph 트랜잭션 하나 = `replaceRange` 한 번 = 되돌리기 한 단계다. 문서를 **갈아 끼우는** 경로(다른 리비전 열기)는 `loadText`로 교체 자체를 이력에 남기지 않고 앞 문서의 단계까지 버린다 — 되돌리기로 이전 리비전 텍스트에 닿을 수 없다. 같은 문서 안의 전체 범위 교체(업그레이드 응답 적용·AI 제안 적용)는 되돌릴 수 있어야 하므로 그대로 `replaceRange` 한 단계다 |
...
>>>>>>> 86ce099c (fix(ide): 같은 문서 전체 교체를 격리된 한 단계로 통일(P1-02 리뷰))
