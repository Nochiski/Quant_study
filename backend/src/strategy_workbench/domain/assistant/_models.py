...
class Usage:
    ...
    그 빼기가 음수가 되는 경우(공급자가 우리가 모르는 방식으로 계약을 바꿨을 때)에만 adapter가
    `input_tokens`를 0으로 깎고 경고를 남긴다. 그 한 경우에는 세 칸의 합이 공급자가 보고한
    총입력보다 크다. 값 자체는 음수가 아니므로 이 타입의 계약은 깨지지 않지만, 합을 청구서와
    맞대는 쪽은 그런 이력이 있을 수 있음을 알아야 한다.
    ...
    def total_input_tokens(self) -> int:
        ...
        **저장·전송 필드가 아니다.** 저장하면 성분과 합이 어긋난 이력이 생길 수 있고, 그때
        ...
        공급자가 보고한 총입력과 이 값이 어긋날 수 있는 경우는 하나뿐이다 — adapter가 성분을 빼다
        음수를 만나 0으로 깎은 이력(위 클래스 docstring). 그때는 이 값이 더 크고, 그 사실은
        adapter의 경고 로그에만 남는다.
        """
