...
| §3-⑨ `n_dedup` 출처 (S05, 09-05) | "dedup 은 `_meta.n_dedup` 이 낸다" | 프레임 `build.py` 가 `_meta.n_dedup` 을 0 으로 고정하므로 `corp_event` 는 산출 컬럼 `n_src_rows` 로 접힌 수를 싣고 EG1 좌변을 `count(out) + Σ(n_src_rows−1)` 로, `n_dedup` 은 `EG3_corp_event` metric 으로 낸다. 우변은 등록표 `rules_s05.SOURCES`(= `_reg_corp_event_source`) 합. reject 후보는 접지 않고 각각 격리해 `Σ원천 = Σ_out n_src_rows + n_reject` 가 성립한다 | DESIGN §4-2 · P21 (34 = 10 + 18 + 6) |
