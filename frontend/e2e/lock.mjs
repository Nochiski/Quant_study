 ...
 * `rename` 은 대상이 있으면 실패하므로(Windows 는 항상, POSIX 는 비어 있지 않은 디렉터리)
 * 동시에 올리려는 둘 중 하나만 성공한다. 잡는 마지막 동작이 원자적 rename 하나라
 * "회수했다가 다시 mkdir" 사이의 창이 없다(1차 리뷰 DEFECT-P105-002).
 * @param {string} path
 * @param {number} pid
 */
...
export const tryAcquireLock = (
  ...
  if (publishLock(path, pid)) return true;
  ...
  return tryAcquireLock(path, pid, true, now);
};
